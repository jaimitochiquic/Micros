/*
 * File:   Codigo_Practica6_Parte2.c
 * Author: JAIME CHIQUI
...
void main(void) {
    ...
    char rx;
    unsigned char pos = 0;
    unsigned char echo = 1;  // 1 = ON, 0 = OFF
    ...
    while(1){    
        tecla = LeerTeclado();
         if(tecla != 0xFF){
            ...
            UART_Write(tx);         // ESCRITURA USART
            rx = USART_Read();      // LECTURA RECEPTOR
            // >> 32 caracteres, limpia y se escribe nuevamente
            if(pos >= 32){   
                LCD_Comando(0x01); // Clear LCD
                pos = 0;
            }
            // Mostrar en LCD, Tecla presionada
            if(echo){
                LCD_Caracter(tx);   // transmitido
                pos ++;       // +1: tx, si se muestra
            }
            LCD_Caracter(rx);   // solo recibido
            pos++;      // +1: rx
  
            if(pos == 16) LCD_Linea2(); // >> 16 caracteres, segunda linea
        }
