...
// ************************************************************
//          EEPROM - Memoria a guardar contraseña
// ************************************************************
    ...
    while(1){    
        ...
        tecla = LeerTeclado();
         if(tecla != 0xFF){
            ...
            DelayXms(20); // antirrebote
            if(tecla <= 9){ // solo acepta numeros
                tx = TeclaASCII(tecla); // TRANSMISOR
                if(indice < 3){ // solo 3 caracteres de contraseña
                    password[indice] = tx;
                    LCD_Caracter('*');
                    indice++;
                }
            ...
            if(tecla==14){          // *
                if(indice>0){
                    indice--;
                    LCD_Comando(0x10);   // cursor izquierda
                    LCD_Caracter(' ');
                    LCD_Comando(0x10);
                }
            }
            if(tecla==13 && indice==3){
                if(PasswordCorrecta(password, clave)){
                    ...
                    DelayXms(2000);
                    LCD_DosLineas("Remote Password:","");
                    estado = 1; // permitir cambio de clave
                } else{
        ...
        if(estado==1 && estadoAnterior != 1){
            ...
            password[1]=0;
            password[2]=0;
            estado=2; // modo Nueva contraseña
        } 
        if(estado == 2){
            tecla = LeerTeclado();
            ...
            if(tecla != 0xFF && tecla != teclaAnterior){
                teclaAnterior = tecla;
                while(LeerTeclado() != 0xFF); // esperar liberación
                DelayXms(20);
