 ...
 * File:   Pr4_Parte2.c
 * Author: JAIME CHIQUI
 * Created on 4 de junio de 2026, 9:35
 */
//  PRACTICA 3 - PARTE 2: RELOJ HH:MM (Sim Microondas)
...
// Flags, HH:SS y contadores de ISR
volatile unsigned int cntISR = 0;   // cuenta int. para 1 s
int Hora  = 0;
int Minutos = 0;
char reloj_activo = 0;    // 0 = ajuste (parpadea), 1 = fija (reloj)
...
void ActualizarDisplays(void){
    if (modo_display){ //Mostrar palabra rEL
        datos[0] = rEL[0]; // D1: r
        datos[1] = rEL[1]; // D2: E
        datos[2] = rEL[2]; // D3: L
...
void main(void){
    ...
    int btn_A_cnt = 0; // Botones col 4 Teclado
    int btn_B_cnt = 0;
    int btn_C_cnt = 0;
    int btn_D_cnt = 0;
    char ing_btn_A = 0; // Presionar A para usar reloj
    ...
    while(1){        
        ...
        if(tecla == 10 && !reloj_activo){ 
            ...
            if(btn_A_cnt == 1){
                ...
                modo_display = 0; Hora = 0; Minutos = 0; //0000
                ActualizarDisplays();
                ing_btn_A = 1;
            }
        } else {
        ...
        // ---- Tecla D: Fijar / Setear Hora ----
        if(tecla == 13 && !reloj_activo && ing_btn_A){
            btn_D_cnt++;
            if(btn_D_cnt == 1){
                Buzzer(); reloj_activo = 1;   // reloj
