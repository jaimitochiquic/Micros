...
# 2 "<built-in>" 2
# 1 "Codigo_Practica6_Parte3.c" 2
# 10 "Codigo_Practica6_Parte3.c"
...
# 73 "Codigo_Practica6_Parte3.c" 2
...
# 137 "Codigo_Practica6_Parte3.c"
    LATAbits.LATA1 = (dato & 0x10)>>4;
    LATAbits.LATA2 = (dato & 0x20)>>5;
    LATAbits.LATA3 = (dato & 0x40)>>6;
    LATAbits.LATA4 = (dato & 0x80)>>7;

    BitEN();

    LATAbits.LATA1 = (dato & 0x01);
    LATAbits.LATA2 = (dato & 0x02)>>1;
    LATAbits.LATA3 = (dato & 0x04)>>2;
    LATAbits.LATA4 = (dato & 0x08)>>3;
...
char LeerTeclado(void) {
    ...
    switch (PORTB) {
        ...
        case 0b01111110: tecla = 10; break;
    }
    ...
    switch (PORTB) {
        ...
        case 0b10111110: tecla = 11; break;
    }
    ...
    switch (PORTB) {
        ...
        case 0b11011110: tecla = 12; break;
    }
    ...
    switch (PORTB) {
        ...
        case 0b11100111: tecla = 14; break;
        case 0b11101011: tecla = 0; break;
        case 0b11101101: tecla = 15; break;
        case 0b11101110: tecla = 13; break;
    }
...
char PasswordCorrecta(char *ingresada, char *guardada){
    for(char i = 0; i < 3; i++){
        if(ingresada[i] != guardada[i]) return 0;
    }
    return 1;
}
void VerificarPassword(char *password,char *indice,char tecla){
    if(*indice!=3) return;
    ...
    UART_Write(password[0]);
    UART_Write(password[1]);
    UART_Write(password[2]);
    *indice=0;
}
...
void EEPROM_InitPassword(){
    if(EEPROM_ReadByte(0)!= '1' ||
       EEPROM_ReadByte(1)!= '2' ||
       EEPROM_ReadByte(2)!= '3'){

        EEPROM_WriteByte(0,'1');
        EEPROM_WriteByte(1,'2');
        EEPROM_WriteByte(2,'3');
    }
}
void LeerPasswordEEPROM(char *clave){
    clave[0]=EEPROM_ReadByte(0);
    clave[1]=EEPROM_ReadByte(1);
    clave[2]=EEPROM_ReadByte(2);
}
void GuardarPasswordEEPROM(char *clave){
    EEPROM_WriteByte(0,clave[0]);
    EEPROM_WriteByte(1,clave[1]);
    EEPROM_WriteByte(2,clave[2]);
}



void __attribute__((picinterrupt(("high_priority")))) MyHighPriorityIsr(void);
...
void main(void) {
    ...
    char teclaAnterior = 0xFF;

    Configuracion_inicialPIC();
    ...
    while(1){
        if(RCIF){
            comando = USART_Read();
            if(comando=='V'){
                recibida[0]=USART_Read();
                recibida[1]=USART_Read();
                recibida[2]=USART_Read();

                if(PasswordCorrecta(recibida,clave)){
                    UART_Write('A');
                }else{
                    UART_Write('X');
                }
                ...
                GuardarPasswordEEPROM(recibida);
                LeerPasswordEEPROM(clave);
                UART_Write('B');
            }
        }
        tecla = LeerTeclado();
         if(tecla != 0xFF){
            ...
            if(tecla==13 && indice==3){
                if(PasswordCorrecta(password, clave)){

                    LCD_DosLineas("OK!","Correcta");
                    DelayXms(2000);
                    LCD_DosLineas("Remote Password:","");
                    estado = 1;
                } else{

                    LCD_DosLineas("Clave","Incorrecta");
                    DelayXms(2000);
                    LCD_DosLineas("Remote Password:","");
                    estado = 0;
                }
        ...
        if(estado==1 && estadoAnterior != 1){
            LCD_DosLineas("OK! New","Password?");
            DelayXms(2000);
            LCD_Comando(0x01);
            LCD_DosLineas("OK! New","");
            LCD_Linea2();
        ...
        if(estado == 2){
            ...
            if(tecla != 0xFF && tecla != teclaAnterior){
                teclaAnterior = tecla;
                while(LeerTeclado() != 0xFF);
                DelayXms(20);
                ...
                if(tecla == 13 && indice == 3){
                    GuardarPasswordEEPROM(password);
                    DelayXms(20);

                    LeerPasswordEEPROM(clave);

                    LCD_DosLineas("Clave","Guardada");
                    DelayXms(2000);
        ...
        if(estado==3){
            rx=USART_Read();
            if(rx=='B'){
                LCD_DosLineas("New Password","set!");
...
void __attribute__((picinterrupt(("low_priority")))) MyLowPriorityIsr(void){
